...
export default {
  ...
  methods: {
    ...
    makePagination (data) {
      ...
      this.next_page_url = (this.current_page === this.last_page) ? "" : utils.getNestedValue(data, this.config.remote_next_page_url);
      this.prev_page_url = (this.current_page === 1) ? "" : utils.getNestedValue(data, this.config.remote_prev_page_url);
    },
    initConfig(){
